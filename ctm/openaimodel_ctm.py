...
class OpenAIUNetModel(nn.Module):
    def __init__(self, **kwargs):
        """
        A wrapper for the UNetModel to make it compatible with a different forward function signature.
        
        :param kwargs: Keyword arguments passed to initialize the UNetModel.
        """
        super().__init__()
        self.unet = UNetModel(**kwargs)

    def forward(self, x, noise_labels, noise_labels_s, class_labels, augment_labels=None, mixture=None):
        """
        Modified forward function to handle additional arguments.
        
        :param x: Input tensor [N x C x ...].
        :param noise_labels: Unused in the original UNetModel, but accepted for compatibility.
        :param noise_labels_s: Optional secondary noise labels (unused here).
        :param class_labels: Labels for class-conditional generation.
        :param augment_labels: Optional labels for data augmentation (unused here).
        :param mixture: Optional mixture inputs (unused here).
        :return: Output tensor [N x C x ...].
        """
        # Concat Mixture if mixture is given
        x = x if mixture is None else th.cat([x, mixture], dim=1) # concatinate mixture if it's present
        
        # Map the new arguments to the original UNetModel's forward inputs
        timesteps = noise_labels  # Assume `noise_labels` represent the timesteps
        s = noise_labels_s  # Secondary timesteps
        y = th.argmax(class_labels, dim=1).to(dtype=th.int64)   # Class labels for conditional generation
             
        return self.unet(x, timesteps=timesteps, y=y, s=s)
