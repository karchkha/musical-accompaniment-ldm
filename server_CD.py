...
def load_network(unused_addr):
    global latent_diffusion, stemidx_to_inpaint, steps, tensor, MSAProc, latent, mask, config, package_size, filename, percentage, diffusion_sampler, diffusion_schedule, pr_win_mul, CAE
...
def predict(*args):
    ...
    with torch.no_grad():
       ...
       # Create a zero-initialized feature tensor for batch size 1
        ...
        model_kwargs = {}
        model_kwargs["class_labels"] = current_features
        model_kwargs["augment_labels"]= None
