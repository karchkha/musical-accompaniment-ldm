...
def _track_metadata(track, sources, normalize=True, ext=EXT):
    ...
    for source in sources + [MIXTURE]:
        ...
        if source == MIXTURE and not file.exists():
            ...
            if would_clip:
                assert ta.get_audio_backend() == 'soundfile', 'use soundstretch --version=soundfile'
...
class Wavset:
    ...
    def __getitem__(self, index):
        for name, examples in zip(self.metadata, self.num_examples):
            ...
            if meta['samplerate'] != self.samplerate:
                resampler = ta.transforms.Resample(orig_freq=meta['samplerate'], new_freq=self.samplerate)
                example = resampler(example)
            if self.normalize:
