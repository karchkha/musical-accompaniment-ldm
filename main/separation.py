...
def separate_dataset(
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        # List of names for each tensor
        stems = ["bass", "drums", "guitar", "piano"]

        # Create a dictionary from the two lists
        tracks_dict = dict(zip(stems, tracks))
        ...
        for i in range(num_samples):
            ...
            # Remove the last level of the path
            parent_path = save_path.parent

            # Add a new segment to the path
            new_save_path = parent_path / "original_msdm"

            # Use this new path for chunk operations
            chunk_path = new_save_path / f"{chunk_id}"
            chunk_path.mkdir(parents=True, exist_ok=True)
            
            save_separation(
                separated_tracks={stem: sep[i] for stem, sep in tracks_dict.items()},
                sample_rate=dataset.sample_rate,
                chunk_path=chunk_path,
            )
            ...
            # Add a new segment to the path
            new_save_path = parent_path / "mixture"

            # Use this new path for chunk operations
            chunk_path = new_save_path / f"{chunk_id}"
            chunk_path.mkdir(parents=True, exist_ok=True)
            
            torchaudio.save(chunk_path / f"mixture.wav", mixture[i].cpu(), sample_rate=dataset.sample_rate)
